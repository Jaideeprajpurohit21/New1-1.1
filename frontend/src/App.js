...
const LuminaApp = () => {
  ...
  const [uploadingReceipt, setUploadingReceipt] = useState(false); // while uploading
  ...
  // Fetch receipts from API with enhanced error handling
  ...
  const fetchReceipts = useCallback(async (search = '', category = '') => {
    ...
    try {
      ...
      console.log(`Ì†ΩÌ≥• Fetching receipts from: ${API_BASE_URL}${url}`);
      ...
      if (data && Array.isArray(data)) {
        ...
        console.log(`‚úÖ Successfully loaded ${data.length} receipts`);

        

        if (error) {

          showNotification('‚úÖ Receipts loaded successfully!', 'success');
        ...
        showNotification('‚ö†Ô∏è Unexpected data format received from server', 'warning');
  ...
  // Stubbed billing info loader (kept API compatible, but safe)

  const loadBillingInfo = async () => {

    try {

      // Backend billing routes may not be active in public demo
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      ...
      console.log('Ì†ΩÌ∫Ä Initializing Lumina App...');

      console.log('Ì†ΩÌ¥ó API Base URL:', API_BASE_URL);
  ...
  const handleReceiptUpload = async (file, category = 'Uncategorized') => {
    ...
    try {
      ...
      const uploadedReceipt = response;
      ...
      if (error.response?.status === 402) {
        ...
        showNotification('Ì†ΩÌ≥ä Receipt limit reached! Time to upgrade to Pro.', 'warning');

      } else {

        const errorMessage = 'Failed to upload receipt.';
  ...
  const exportReceipts = async (filters = null) => {
    ...
    try {
      ...
      // Use axios for blob download here

      const response = await axios.post(

        `${API_BASE_URL}/receipts/export/csv`,
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Auto-Detect">Ì†æÌ¥ñ Auto-Detect</SelectItem>

                    <SelectItem value="Meals & Entertainment">Ì†ºÌΩΩÔ∏è Meals & Entertainment</SelectItem>

                    <SelectItem value="Groceries">Ì†ΩÌªí Groceries</SelectItem>

                    <SelectItem value="Transportation & Fuel">Ì†ΩÌ∫ó Transportation & Fuel</SelectItem>

                    <SelectItem value="Office Supplies">Ì†ΩÌ≥é Office Supplies</SelectItem>

                    <SelectItem value="Shopping">Ì†ΩÌªçÔ∏è Shopping</SelectItem>

                    <SelectItem value="Utilities">‚ö° Utilities</SelectItem>

                    <SelectItem value="Healthcare">Ì†ºÌø• Healthcare</SelectItem>

                    <SelectItem value="Travel">‚úàÔ∏è Travel</SelectItem>

                    <SelectItem value="Uncategorized">Ì†ΩÌ≥Ç Uncategorized</SelectItem>
